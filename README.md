...
Inspired by SHA-256 with double state row, larger block size, and AES S-box.
...
- Padding – SHA-256 style 0x80 + zeros + 64-bit length (padded length % 256 == 248)
...
- Nonlinear mixing – Ch, Maj, custom sigma0/sigma1, and AES S-box
...
Do not use for passwords, digital signatures, file integrity, or any
...
The input data is transformed into:

    b"sesht5er" + data + b"gormitimhaaa"
...
Two rows of 8 state words (h[0:8] and h[8:16]) are set to the first
 eight SHA-256 constants (repeated twice).

4. Block processing (256 bytes at a time)
...
sub_bytes(word)
    Splits a word into 4 bytes, applies AES S-box to each, reassembles
...
sigma0(x)
    rotl(x,7) ^ rotl(x,18) ^ (x >> 3)

sigma1(x)
    rotl(x,17) ^ rotl(x,19) ^ (x >> 10)

powerful_hash(data)
    Main hash function implementing Ozhash, returns a hex string

--------------------------------------------------
Why the Name "Ozhash"?
--------------------------------------------------

The name is a short, distinctive identifier for this experimental hash.
The original code referred to "powerful hash", but the algorithm is now branded as Ozhash.
